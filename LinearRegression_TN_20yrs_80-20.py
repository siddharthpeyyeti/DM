# -*- coding: utf-8 -*-
"""
Created on Sat Apr 25 17:03:33 2020
...
###Using functions defined to perform linear regression
    
errors = [] #list of RMSE for each state

###10 YEARS AT A TIME FOR TAMIL NADU
for i in range(len(states)):
    if (states[i]=='Tamil Nadu'):
        break

###get 10 values at a time
while (not l_state_dfs[i].empty):
    #first 10 rows of df
    ten_rows = l_state_dfs[i].head(20)
    full=ten_rows
    #take 1st 7/10 rows
    ten_rows=ten_rows.head(int(len(ten_rows)*(80/100)))
    #do linear regression on seven rows
    coeffs = lin_reg(ten_rows)
    w0 = coeffs[0]
    w1 = coeffs[1]
    #predict for ten rows
    full['Predicted_Rainfall'] = w0 + w1 * full['YEAR']
    #RMSE calculation for last 3 rows
    test_vals = full.tail(int(len(l_state_dfs[i])*(20/100))+1)
    rmse = cal_rmse(test_vals)
    errors.append(rmse)
    #remove the 10 rows from the dataframe and perform the same operations on next ten rows
    l_state_dfs[i] = l_state_dfs[i].iloc[20:]
